...
class Controller implements Crud
{
    ...
    public function __construct($model, $name = '', $only = '', $api = false, $parent = false)
    {
        ...
        $this->requestClassSuffix = config('karl.laracrud.request.classSuffix', 'Request');
    ...
    protected function globalVars()
    {
        ...
        return [
            ...
            'routePrefix' => config('karl.laracrud.route.prefix', ''),
            'apiRequest' => '{}',
            'apiResponse' => '{}',
        ];
    }
    ...
    protected function buildMethods()
    {
        ...
        foreach ($this->only as $method) {
            ...
            if ($filePath = $tempMan->getFullPath('controller/' . $this->template . '/' . $method . '.txt')) {
                ...
                if (true == config('karl.laracrud.controller.documentation') && 'web' !== $this->template) {
    ...
    {
        $transformerNs = $this->getFullNS(config('karl.laracrud.transformer.namespace', 'Transformers'));
        $suffiex = config('karl.laracrud.transformer.classSuffix', 'Transformer');
    ...
    protected function parseModelName()
    {
        ...
        $namespace = config('karl.laracrud.view.namespace');
        ...
        if ($table->hasFile()) {
            ...
            foreach ($columns as $column) {
                $temp = new TemplateManager('controller/upload.txt', [
                    ...
                    'imageUploadDisk' => config('karl.laracrud.image.disk', 'public'),
                ]);
                $retTemp .= $temp->get();
            }
        }
