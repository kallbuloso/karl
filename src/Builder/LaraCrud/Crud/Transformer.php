...
class Transformer implements Crud
{
    ...
    public function __construct(Model $model, $name = false)
    {
        $this->model = $model;
        $this->name = $name;
        $this->namespace = $this->getFullNS(config('karl.laracrud.transformer.namespace'));
        $this->reflectionClass = new \ReflectionClass(get_class($model));
        $this->modelName = !empty($name) ? $name : $this->reflectionClass->getShortName() . config('karl.laracrud.transformer.classSuffix', 'Transformer');
        $this->makeIncludes();
    }
    ...
    private function makeTransformer($class, $modelRef)
    {
        ...
        $transformerName = $shortName . config('karl.laracrud.transformer.classSuffix');
        $transformerClass = $this->getFullNS(config('karl.laracrud.transformer.namespace') . '\\' . $transformerName);
